...
from core.decorators import session_login_required
...
def upload_bloodtests(request):
    ...
    if request.method == "POST":
        ...
        if "upload" in request.POST and request.FILES.get("file"):
            ...
            if file.content_type in [
                ...
                return redirect('upload_bloodtests')
            else:
    ...
    if request.session.pop('upload_success', False):
...
def bloodtest_preview(request, file_id):
    ...
    if file_content:
        return FileResponse(file_content, content_type='application/pdf')
...
def bloodtest_download(request, file_id):
    ...
    file_content = medical_file.get_file()
    if file_content:
        return FileResponse(file_content, as_attachment=True, filename=medical_file.file_name)
